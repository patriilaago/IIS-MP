...
import uo.mp.newsstand.comparator.OrderComparator;
...
public class Newsstand {
	...
	private final static int HIGH_STOCK = 10;
	
	private List<Publication> publications = new LinkedList<>();
	private List<Order> orders = new LinkedList<>();
	private Logger logger = new Logger(UserInterface.LOG_FILE);
	...
	private void checkFileNameLength(String inFileName) throws InvalidNameException {
	...
	public List<Order> createOrders() {
		for (Publication publication: publications) {
			if(publication.getStock()<HIGH_STOCK) {
	 ...
	 */
	public void saveOrdersToFile(String fileName) throws IOException, InvalidNameException {
	 ...
	 * @throws IOException 
	 */
	public void importPublicationsFromZipFile(String fileName) throws NewsStandException, InvalidNameException, IOException {
		checkFileNameLength(fileName);
		List<String> lines;
		try {
			lines = new ZipFileUtil().readLines(fileName);
		} catch (FileNotFoundException e) {
			throw new NewsStandException(e.getMessage());
		}
		List<Publication> fileProducts = new PublicationParser().parse(lines);
		addPublications(fileProducts);
	}	
